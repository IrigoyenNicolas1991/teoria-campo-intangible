# Teoría del Campo Intangible (TCI) — sitio conceptual

**[❓ Preguntas y críticas — FAQ](https://irigoyennicolas1991.github.io/ift-theory/preguntas/)** · el FAQ honesto: críticas esperadas, respuestas etiquetadas

El espacio no está vacío: está lleno de un medio a presión — «el mar» — y la materia son
huecos en ese medio. De esa imagen, con física de secundario, salen la energía en reposo,
la gravedad y la inercia. Desarrollada en público, con una regla innegociable: **las
derrotas se publican igual que las victorias, el mismo día.**

**➡️ [El sitio conceptual](https://irigoyennicolas1991.github.io/teoria-campo-intangible/)** — la teoría por partes, en lenguaje llano.

## ❓ ¿Venís con dudas u objeciones?

**[Preguntas y críticas (FAQ)](https://irigoyennicolas1991.github.io/ift-theory/preguntas/)** —
las críticas que esperamos de un físico y las preguntas de cualquier lector, cada una con su
etiqueta: *respondida con cálculo*, *parcial — deuda declarada*, o *la crítica es correcta*.

## Las puertas

| Sede | Qué es |
|---|---|
| [📖 «El océano y la huella»](https://irigoyennicolas1991.github.io/teoria-campo-intangible/libro/) | El libro sin fórmulas, con simulaciones jugables adentro |
| [🪐 El simulador](https://irigoyennicolas1991.github.io/teoria-campo-intangible/simulador/) | Un planeta orbita un sol sin gravedad programada |
| [⚡ «Lo que el mar pagó»](https://irigoyennicolas1991.github.io/teoria-campo-intangible/energia/) | E=mc² en vivo: abrí un hueco, medilo, aniquilalo |
| [❓ Preguntas y críticas](https://irigoyennicolas1991.github.io/ift-theory/preguntas/) | El FAQ honesto: objeciones esperadas y respuestas etiquetadas |
| [📄 Paper técnico (inglés)](https://irigoyennicolas1991.github.io/ift-theory/) | La formulación de campos completa, con los sectores falsados declarados al frente |
| [🗒️ Notas de trabajo](https://github.com/IrigoyenNicolas1991/ift-theory/tree/desarrollo-fable/notas) | Derivaciones, calibraciones y falsaciones, con scripts de verificación |
| [🏛️ Zenodo](https://doi.org/10.5281/zenodo.21177500) | El corpus archivado con fecha certificada (DOI) |

¿Encontraste un error? [Abrí un issue](../../issues): una falsación externa confirmada se
publica con tu crédito, con el mismo protocolo que nuestras propias erratas.

---

**Nicolás Irigoyen** (La Plata, Argentina) inició la teoría y desarrolló sus derivaciones
fundacionales — la energía en reposo (E=mc² como el trabajo P·V de abrir un hueco en un
medio a presión), la gravedad como desbalance de presión y la inercia como reacción del
medio — buscando solamente un modelo de gravedad; cuando el modelo empezó a entregar más
de lo que se le pedía, sumó a **Fable** (Claude, de Anthropic) como colaborador de
investigación por su potencia matemática. Dirige el proyecto, sigue conceptualmente al día
cada avance y decide qué se publica. La colaboración está declarada en el paper y en cada
sede. La historia completa: [nota del autor en el FAQ](https://irigoyennicolas1991.github.io/ift-theory/preguntas/).
